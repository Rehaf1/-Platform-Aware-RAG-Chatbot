...
## ~~3. Cross-format duplicate check blocks legitimate reindex~~ ✅ FIXED 2026-08-10
Fixed by adding a `skip_duplicate_check` parameter to `_ingest_file`, set to `True` only when called from `reindex_document` — uploads still correctly check for duplicates.
